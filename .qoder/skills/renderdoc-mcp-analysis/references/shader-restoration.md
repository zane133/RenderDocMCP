# Reference — Decompiled shader restoration

Applies to 3Dmigoto HLSL dumps (paste) and HLSL reconstructed from MCP
disassembly. "Patch step" = Step 4 register-faithful HLSL; "readable pass" =
Step 5.

## RenderDoc Apply Changes constraints

- Compiler is effectively FXC/DXBC-oriented for D3D11 captures.
- Resource binding is by **register**, not HLSL name — renaming `t1` → `texNoise`
  is fine if `: register(t1)` stays.
- Changing `cbuffer` packing (`float4 cb1[15]` → individually named fields
  without `packoffset`) can silently break layouts. Prefer:
...
- Input signature must match the paired stage. 3Dmigoto often splits one VS
  `float3 o6` into PS `float2 v6 : TEXCOORD5` + `float w6 : TEXCOORD6`. Keep
  that split.
- Unused parameters (`SV_Position`, extra TEXCOORDs) may still be required for
  signature matching after path removal.

## `#define cmp -`

3Dmigoto emits comparisons as `cmp(expr)` expecting **true → -1, false → 0**
(DXBC-style).
...
3Dmigoto `t125[i].val[32/4]` style indices are **byteOffset/4** — keep as-is
(or comment the byte origin); do not "simplify" to wrong indices.
...
…copy the **Apply-verified register path** into the helper. Do not re-derive
cubic weights from memory; small mad order mistakes break the look.
...
- [ ] Worked from **this paste / this disassembly**, not an old readable file
- [ ] Patch step only: cmp / SV_* / asuint+ubfe / SampleGrad `.xy`
- [ ] Stopped for user Apply + visual match before the readable pass
...
- [ ] Dense VT/volume helpers still match the verified math
