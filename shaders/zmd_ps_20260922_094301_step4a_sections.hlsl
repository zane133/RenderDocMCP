// ZMD / Endfield PS - Step 4a: section map + readable resource names
// Source dump: 3Dmigoto 2026-09-22 09:43:01
// Step 1 Apply-verified (user OK). This pass only:
//   - readable Texture*/SamplerState names (registers unchanged)
//   - section comments in main
// Register math / op order identical to step1_renderdoc.hlsl.
//
        ...
        if (r16.w == 0) {
          ...
          if (r16.w != 0) {
            if (r12.z == 0) {
              ...
              r18.w = cmp(r18.w < 0);
              bitmask.w = ((~(-1 << 31)) << 1) & 0xffffffff;  r16.w = (((uint)r16.w << 1) & bitmask.w) | ((uint)r18.w & ~bitmask.w);
              r18.w = (uint)r16.w >> 1;
