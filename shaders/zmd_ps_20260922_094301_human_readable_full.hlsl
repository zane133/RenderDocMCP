// =============================================================================
// ZMD / Endfield Character PS — human-readable FULL reconstruction
// =============================================================================
// Source dump: 3Dmigoto 2026-09-22 09:43:01
// Lineage: Step1 (Apply-OK) → 4a..4d → Step5 → this file
//
// All paths KEPT (volume SH / detail / local lights / fog). Dense blocks stay
// register-faithful. Named resources + stage recipes for reading / analysis.
//
// ---- Pipeline (o0 = colour, o1 = motion / flags) ----
//  1 ViewDir                         → EvalViewDir
//  2 Per-instance cb1 (TEXCOORD8)
//  3 Material t14..t17               → EvalUnpackNormalMap
//  4 TBN → world normal              → EvalShadingNormal
//  5 Volume SH probes t4..t9         [STEP_VOLUME_PROBES]  register body
//  6 Detail noise/normal t12/t13     [STEP_DETAIL]         register body
//  7 F0 split + motion               → EvalDiffuseSpecF0 / EvalMotionVector
//  8 Sun + RampMap t10 wrap lighting
//  9 Rim / 边缘光 cb0[194..195]       [STEP_RIM]
// 10 Emissive + reflection cube      [STEP_IBL_EMISSIVE]
// 11 Clustered local lights          [STEP_LOCAL_LIGHTS]   register body
// 12 Color grade cb5[3..8]           [STEP_COLOR_GRADE]
// 13 Exposure + volumetric fog t20   [STEP_FOG]            register body
//
// Interpolators (do NOT const-copy; broke rim once):
//  v0 SV_Position | v1 UV | v2 worldPos | v3 geomN | v4 tangent+sign
//  v5/v6 motion | v7 detail axis | v8 detail UV | v9 instanceId | v10 frontFace
//
// Apply Changes risks: shared PS object affects all draws using this shader.
// =============================================================================

// ===== Analysis switches =====
#define STEP_VOLUME_PROBES 1  // 0 → no SH probe path (else-branch identity)
#define STEP_DETAIL        1  // 0 → skip detail overlay (keep base shading normal)
#define STEP_RIM           1  // 0 → rim term factor 0 (do NOT peak the wrap)
#define STEP_IBL_EMISSIVE  1  // 0 → no emissive / reflection-cube add
#define STEP_LOCAL_LIGHTS  1  // 0 → skip tiled local-light loop
#define STEP_COLOR_GRADE   1  // 0 → skip cb5 color grade
#define STEP_FOG           1  // 0 → exposed color only (no atmosphere)

#define DEBUG_VIS 0
// 0 final
// 1 baseColor (post cb5 tint)
// 2 shadingNormal *0.5+0.5
// 3 materialMask.xyz
// 4 litDirect (r19 before rim/probe GI combine into r17)
// 5 rim term only (r21*r23)
// 6 preFog lit (r15 after lights / grade)
// 7 1-|N·V| saved at rim (facing≈黑, 边缘≈白; not end-of-shader r2)
...
//   t10 RampMap             - PS t10 ramp (边缘光 / lighting wrap)
//   t11 SpecularBRDFLUT
//   t12 DetailNoise
//   t13 DetailNormal
//   t14 BaseColor
...
void EvalViewDir(float3 worldPos, out float3 toCam, out float3 camFwdZ, out float toCamLenSq, out float rcpLen, out float3 viewDir)
{
  toCam = cb0[44].xyz + -worldPos;
  camFwdZ.x = cb0[0].z;
  camFwdZ.y = cb0[1].z;
  camFwdZ.z = cb0[2].z;
  float3 bent = camFwdZ + -toCam;
  toCam = cb0[86].www * bent + toCam;
  toCamLenSq = dot(toCam, toCam);
  rcpLen = rsqrt(max(9.99999994e-09, toCamLenSq));
  viewDir = rcpLen * toCam;
}
...
void EvalShadingNormal(float3 nTS, float nz, float3 geomN, float4 tangent, uint isFrontFace, out float3 nMapped, out float3 nWorld, out float3 geomNSigned, out float faceSign)
{
  ...
  float len = dot(nMapped, nMapped);
  len = max(1.17549435e-38, len);
  len = rsqrt(len);
  nMapped = nMapped * len;
  nWorld = nMapped * faceSign;
  len = dot(geomN, geomN);
  len = rsqrt(len);
  geomNSigned = geomN * len;
  geomNSigned = geomNSigned * faceSign;
}

void EvalDiffuseSpecF0(float3 albedo, float metallic, float roughness, inout float3 colSat, out float dielectF, out float3 diffuse, out float3 specF0)
{
  ...
  out float4 o1 : SV_Target1)
{
  ...
  // CB aliases (inferred). Arrays kept as float4 cbN[] for packing.
  // Do NOT copy interpolators into locals — that previously dropped rim.
  ...
  const float2 rimAxisZW = cb0[195].yx;   // builds rim tangent axis with sunDir
  const float rimAlbedoMix = cb0[195].z;
  const float rimNdotVWidth = cb0[195].w;
  const float volumeProbeEnable = cb0[187].y; // < 0.5 enables probe path

  // ===== 1. View / camera direction from worldPos =====
  EvalViewDir(v2.xyz, r0.xyz, r1.xyz, r0.w, r1.w, r2.xyz);
  ...
  // r4 baseColor  r5 mask(.x met,.y rough,.z AO,.w extra)  r6 sat  r8 emissive
  // mask (inferred): .x metallic-ish, .y roughness-ish, .z AO/spec, .w extra
  r4.xyzw = BaseColorMap.SampleBias(sampBaseColor, v1.xy, mipBias).xyzw;
  r5.xyzw = MaterialMask.SampleBias(sampMaterial, v1.xy, mipBias).xyzw;
  ...
  r7.xyz = EvalUnpackNormalMap(v1.xy, mipBias, r6.w);
  r8.xyz = EmissiveMap.SampleBias(sampEmissive, v1.xy, mipBias).xyz;
  float3 dbgBaseColor = r4.xyz;
  float3 dbgMask = r5.xyz;
  ...
  {
    float3 nTS = r7.xyz;
    EvalShadingNormal(nTS, r6.w, v3.xyz, v4, v10.x, r7.xyz, r10.xyz, r11.xyz, r3.x);
  }
  float3 dbgShadingN = r10.xyz;
  r12.xy = (uint2)v0.xy;
  r3.y = -cb0[111].x + 1;
  r3.y = cb0[198].w * r3.y + cb0[111].x;
  r3.y = exposureScale * r3.y;
  // ===== 5. Volumetric / probe SH cascade (t4..t9) optional path =====
  r6.w = cmp(volumeProbeEnable < 0.5);
#if !STEP_VOLUME_PROBES
  r6.w = 0; // identity: take else (no probe SH)
#endif
  if (r6.w != 0) {
    r13.xyz = sunDir.xzy * -cb0[212].www + cb0[210].xzy;
    ...
    r7.w = r7.w ? r8.w : 0;
    if (r7.w != 0) {
      r13.xyz = sunDir.xzy * -cb0[212].yyy + cb0[210].xzy;
      r13.xyz = v2.xzy + -r13.xyz;
      ...
      r15.xyz = sunDir.xzy * -cb0[212].zzz + cb0[210].xzy;
...
#if !STEP_DETAIL
  // identity: same as original else (keep r10 shading normal, no detail)
  r7.xyz = r10.xyz;
  r7.w = 0.00999999978;
  r2.w = 0;
#else
  ...
  if (r8.w != 0) {
    ...
    r21.xyzw = DetailNoise.SampleBias(sampBaseColor, r15.zy, mipBias).xyzw;
    r22.xyzw = DetailNoise.SampleBias(sampBaseColor, r15.zw, mipBias).xyzw;
    r22.xyzw = r22.xyzw * r19.zzzz;
    r21.xyzw = r21.xyzw * r19.yyyy + r22.xyzw;
    r22.xyzw = DetailNoise.SampleBias(sampBaseColor, r15.yw, mipBias).xyzw;
    ...
    r22.xyz = DetailNormal.SampleBias(sampBaseColor, r15.zw, mipBias).xyz;
    r15.yzw = DetailNormal.SampleBias(sampBaseColor, r15.yw, mipBias).xyz;
    ...
    r13.y = DetailNormal.SampleBias(sampBaseColor, r22.xy, mipBias).w;
    r13.z = DetailNormal.SampleBias(sampBaseColor, r22.zw, mipBias).w;
    r13.z = r13.z * r16.x;
  ...
  // r13 diffuse albedo  r15 spec F0  r5.w dielectric 1-m*0.96
  EvalDiffuseSpecF0(r4.xyz, r5.x, r5.y, r6.xyz, r5.w, r13.xyz, r15.xyz);
  r3.x = r3.z * r3.z;
  ...
  // recipe: N·sun → RampMap twice; blend by screen AO (t3) + probe GI;
  //         half-vector + SpecularBRDFLUT (t11) → highlight + direct lit
  ...
  r10.w = max(-1, r10.w);
  r10.w = min(1, r10.w);
  // RampMap (t10): U = lighting wrap, V = 0.5
  r25.x = r10.w * 0.5 + 0.5;
  r25.yw = float2(0.5,0.5);
  r26.xyzw = RampMap.SampleLevel(sampLinear, r25.xy, 0).xyzw;
  r10.w = max(r26.x, r26.y);
  r10.w = max(r10.w, r26.z);
  r13.w = min(r26.x, r26.y);
  r13.w = min(r13.w, r26.z);
  r10.w = -r13.w + r10.w;
  r13.w = dot(r10.xyz, sunDir);
  r25.z = r13.w * 0.5 + 0.5; // second ramp tap from N·sun
  r13.w = RampMap.SampleLevel(sampLinear, r25.zw, 0).w;
  r14.w = r22.y * r5.z;
  ...
  r5.y = min(0.5, r5.y);
  // ===== rim / 边缘光 =====
  // axis = sunDir × rimAxis; colour = rimColor * wrap(1-|N·V|) * AO-ish
  // mid block = rational visibility fit (Karis-style); KEEP register body
  r23.y = 0;
  r23.xz = rimAxisZW;
  r26.xyz = sunDir.zxy * r23.xyz;
  r23.xyz = sunDir.yzx * r23.yzx + -r26.xyz;
  r13.w = dot(r23.xyz, r23.xyz);
  r13.w = rsqrt(r13.w);
  r23.xyz = r23.xyz * r13.www; // rim light axis
  r13.w = dot(r2.xyz, r10.xyz); // N·V (viewDir · shadingNormal) — r2 still viewDir here
  float dbgOneMinusAbsNdotV = 1.0 + -abs(r13.w); // capture before fog overwrites r2
  ...
  r21.xy = rimNdotVWidth.xx * float2(-0.600000024,-0.399999976) + float2(0.800000012,0.899999976);
  r3.y = r21.y + -r21.x;
  r5.y = r26.x + -r21.x;
  r3.y = 1 / r3.y;
  ...
  r3.y = r3.y * r3.y;
  r3.y = r5.y * r3.y;
  r21.xyz = rimColor * r3.yyy;
  r21.xyz = rimIntensity.xxx * r21.xyz;
  ...
  r25.yzw = rimAlbedoMix.xxx * r25.yzw + float3(0.25,0.25,0.25);
  ...
  float3 dbgRim = r21.xyz * r23.xyz;
  float3 dbgLitDirect = r19.xyz;
#if STEP_RIM
  r17.xyz = dbgRim + r17.xyz;
#else
...
#else
  r8.xyz = r17.xyz; // identity: no emissive / IBL
#endif
  r2.w = cmp(r8.w >= r6.w);
            ...
            if (r17.y == 0) {
              ...
              if (cb3[r14.z+6].w == 0) {
                ...
                if (r20.w != 0) {
                  ...
                  r20.w = r20.w * r12.z;
                  r14.x = r20.w * r14.x;
                  r25.yzw = sunDir * r21.zxy;
                  r25.yzw = sunDir.zxy * r21.xyz + -r25.yzw;
                  r26.yzw = sunDir.zxy * r25.yzw;
                  r25.yzw = sunDir.yzx * r25.zwy + -r26.yzw;
...
#else
  // identity: r15 already = r8 (no local lights)
#endif
  ...
  float3 dbgPreFog = r15.xyz;
  // ===== 12. Exposure + volumetric fog / atmosphere (o0) =====
  // fog on  (cb0[198].w < 0.5): transmittance exp + inscatter blend
  // fog off: o0 = exposed lit colour only
  r0.xyz = r15.xyz / exposureScale;
  ...
  if (r2.w != 0) {
    ...
    if (r3.w != 0) {
      ...
      // integer mad -> a*b+c (FXC mad is float-oriented). LCG hash for fog dither.
      r4.xyz = (int3)r12.xyw * int3(0x19660d,0x19660d,0x19660d) + int3(0x3c6ef35f,0x3c6ef35f,0x3c6ef35f);
      r3.w = (int)r4.y * (int)r4.z + (int)r4.x;
      r4.x = (int)r4.z * (int)r3.w + (int)r4.y;
      r4.y = (int)r3.w * (int)r4.x + (int)r4.z;
      r6.x = (int)r4.x * (int)r4.y + (int)r3.w;
      ...
      r1.x = r1.x + r1.z;
      r1.x = min(1, r1.x);
      r6.y = (int)r4.y * (int)r6.x + (int)r4.x;
...
#elif DEBUG_VIS == 7
  // Must use value saved at rim (r2 is clobbered by fog before return).
