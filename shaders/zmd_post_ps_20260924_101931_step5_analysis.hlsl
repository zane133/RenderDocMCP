...
// STEP 5: analysis harness on the Step 4 Apply-verified file.
//
// STEP_* default 1 = original behavior.
// Off = documented identity (no contribution), never "peak of the curve".
// DEBUG_VIS 0 = final; 1..8 = intermediates captured at their producing sites.
// Bindings, signatures, and default-on math stay the same as Step 4.
//
// Working RGB in this dump is stored as BGR via r0.zxy after the first stage.
// DEBUG_VIS converts back with .yzx so the overlay reads as RGB.

#ifndef STEP_SHARPEN
#define STEP_SHARPEN 1   // off: keep center sample (zxy), skip CAS mix
#endif
#ifndef STEP_EXPOSURE
#define STEP_EXPOSURE 1  // off: do not scale by cb0[109].x
#endif
#ifndef STEP_BLOOM
#define STEP_BLOOM 1     // off: bloomBlend identity 0 → exposed color only
#endif
#ifndef STEP_VIGNETTE
#define STEP_VIGNETTE 1  // off: multiply by 1 (no darkening / tint)
#endif
#ifndef STEP_LUT
#define STEP_LUT 1       // off: skip colour LUT, keep linear graded color
#endif
#ifndef STEP_SRGB
#define STEP_SRGB 1      // off: skip linear-to-sRGB transfer
#endif
#ifndef STEP_DITHER
#define STEP_DITHER 1    // off: add 0 (no ordered dither)
#endif
#ifndef DEBUG_VIS
#define DEBUG_VIS 0
// 0 final
// 1 ColorInput RGB
// 2 after sharpen (RGB)
// 3 after exposure (RGB)
// 4 processed bloom source (RGB)
// 5 after bloom compose (RGB)
// 6 vignette factor (xxx)
// 7 after vignette (RGB)
// 8 LUT result (before sRGB)
#endif
  ...
  out float4 o0 : SV_Target)
{
  ...
  float3 visSharpen     = 0;
  float3 visExposed     = 0;
  float3 visBloomSrc    = 0;
  float3 visAfterBloom  = 0;
  float  visVignette    = 1;
  float3 visAfterVig    = 0;
  float3 visLut         = 0;
  float3 visSrgb        = 0;

  // --------------------------------------------------------------------------
  // 1. Contrast-adaptive sharpening
  // --------------------------------------------------------------------------
...
#else
  // Identity: CAS mix amount 0 → only the dump's zxy channel reorder.
  r0.xyz = r0.zxy;
#endif
  o0.w = min(1, r0.w);
  visSharpen = r0.yzx;
...
#else
  // Identity: bloomBlend = 0 → result is the exposed colour only.
  // Do not leave the local highlight / bloom add in (that is the mix, not a mask).
  r0.xyz = r1.xyz;
#endif
  visAfterBloom = r0.yzx;
...
#if !STEP_LUT
  // Identity: no LUT. Restore the pre-LUT linear colour (after vignette).
  // Cannot zero the log encode — that would collapse the image.
  r0.xyz = visAfterVig.xyz;
#endif
  visLut = r0.xyz;
  ...
  o0.xyz = visBloomSrc;
#elif DEBUG_VIS == 5
  o0.xyz = visAfterBloom;
#elif DEBUG_VIS == 6
  o0.xyz = visVignette.xxx;
#elif DEBUG_VIS == 7
  o0.xyz = visAfterVig;
#elif DEBUG_VIS == 8
  o0.xyz = visLut;
