...
void vs_SetMaterial_techMoving(
  ...
  out float4 o0 : SV_Target)
{
  ...
  if (r2.x != 0) {
    ...
    r8.xy = (int2)r3.xw;
    r8.zw = (int2)r4.xx;
    uint pageWord = asuint(t2.Load(r8.xyz).x); // typeless R32 page-table load
    r4.x = (float)((pageWord >> 14) & 1023u); // ubfe 10bit@14 (≡ dump ternary (w<<8)>>22)
    r4.y = (float)((pageWord >> 4) & 1023u);  // ubfe 10bit@4  (≡ dump ternary (w<<18)>>22)
    r4.z = (float)((pageWord >> 24) & 127u);  // ubfe 7bit@24  (≡ dump ternary (w<<1)>>25)
    r3.x = (float)(pageWord & 15u);           // ≡ dump (int)&15 chain, bits flow into exp2 below
    r8.xz = exp2(r3.xx);
    r8.y = r8.z * r3.y;
    r2.zw = r8.xy * r2.zw;
    r2.zw = frac(r2.zw);
    ...
    r2.x = t3.SampleGrad(s0_s, r3.xyw, r4.yz, r2.yz).w; // full ddx/ddy (X3206 fix)
  ...
  if (r0.w != 0) {
    ...
    r0.w = r1.x / cb6[32].w;
    r1.z = r1.y / cb6[22].w;
    r1.y = cmp(cb6[23].x >= w6);
    r3.x = cb6[23].z * w6 + cb6[23].w;
    r3.y = cb6[24].z * w6 + cb6[24].w;
    r3.y = log2(r3.y);
    r3.y = cb6[24].y * r3.y;
    r1.w = r1.y ? r3.x : r3.y;
    r1.y = asfloat(asuint(cb9[2].z)); // bit-preserving mov (DXBC int view) into float reg
    r1.x = r1.y + r0.w; // fadd
