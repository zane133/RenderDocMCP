// EID 7918 pixel shader - Stage 4j readable VT fallback state.
// Source: current D:\Capture\BG3\handSSSS.rdc, shader hash
// 1288ff5a-12d0d3f6-882c1fc8-590970d9, ps_5_0, 283 DXBC instructions.
//
// Stage 4i was Apply-verified against the original draw. This revision only
// names the two VT fallback masks and the captured valid-page tag. Expressions,
// accumulation order, VT math, and resource bindings are intact.
...
PSOutput main(PSInput input)
{
    ...
    float3 materialOverrideAColor =
        r4.z * towardMaterialOverrideA + dyedBaseColor;
    float3 towardMaterialOverrideB =
        (-materialOverrideAColor) + material.materialOverrideB.xyz;
    output.baseColorPhysical.xyz = saturate(
        r4.w * towardMaterialOverrideB + materialOverrideAColor);
